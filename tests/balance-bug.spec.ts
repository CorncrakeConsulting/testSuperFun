...
test.describe('Balance Calculation Bugs', () => {
  ...
  test('should correctly update balance when landing on 2X multiplier', async () => {
    ...
    console.log(`Expected Balance: ${expectedBalance} (${balanceBeforeSpin} - ${betBeforeSpin} + ${winAmount})`);
  ...
  test('should correctly update balance for all multipliers', async () => {
    ...
    results.forEach(result => {
      ...
      console.log(`${status} Slice ${result.sliceIndex} (${result.multiplier}X): Expected ${result.expectedBalance}, Got ${result.actualBalance}`);
    });
    ...
    if (bugs.length > 0) {
      console.log('\n🐛 BUGS FOUND:');
      bugs.forEach(bug => {
        console.log(`   Slice ${bug.sliceIndex} (${bug.multiplier}X): Off by ${bug.actualBalance - bug.expectedBalance}`);
      });
    }
  ...
  test('BUG REPORT: 2X multiplier balance calculation', async () => {
    ...
    for (let i = 0; i < 8; i++) {
      await gamePage.setPlayerData({ balance: initialBalance, bet: betAmount, win: 0 });
    ...
    console.log(`Bug: Balance is ${finalBalance === 1050 ? 'CORRECT' : `INCORRECT (off by ${finalBalance - 1050})`}`);
